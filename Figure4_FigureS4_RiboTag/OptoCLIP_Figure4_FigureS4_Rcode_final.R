...
Base_directory=file.path("~/ruthasinger_github/OptoCLIP/Figure4_FigureS4_RiboTag")
...
mm10_Tx_final <- readRDS(file.path("~/ruthasinger_github/OptoCLIP/RDS_files/mm10_Tx_final.rds"))
mm10_txdb_lengths <- readRDS(file.path("~/ruthasinger_github/OptoCLIP/RDS_files/mm10_txdb_lengths.rds"))
...
RiboTag_TPM=RiboTag_salmon_TPM_mat %>%
  dplyr::mutate(ChR2_5min_rep1_log2FC      = log2(RiboTag_Exp03_M18_ChR2_5min_IP_salmon/RiboTag_Exp03_M18_ChR2_5min_Input_salmon),
                ...
                Control_Control_rep1_log2FC  = log2(RiboTag_Exp03_M26_Control_Control_IP_salmon/RiboTag_Exp03_M26_Control_Control_Input_salmon),
                Control_Control_rep2_log2FC  = log2(RiboTag_Exp03_M27_Control_Control_IP_salmon/RiboTag_Exp03_M27_Control_Control_Input_salmon),
                Control_Control_rep3_log2FC  = log2(RiboTag_Exp04_M38_Control_Control_IP_salmon/RiboTag_Exp04_M38_Control_Control_Input_salmon),
                Control_Control_rep4_log2FC  = log2(RiboTag_Exp04_M39_Control_Control_IP_salmon/RiboTag_Exp04_M39_Control_Control_Input_salmon),
...
RiboTag_TPM_GOI_log2FC=RiboTag_TPM_GOI %>% dplyr::select(c(gene_name,
                                                           ...
                                                           Control_Control_rep1_log2FC,
                                                           Control_Control_rep2_log2FC,
                                                           Control_Control_rep3_log2FC,
                                                           Control_Control_rep4_log2FC,
...
sample_order <- c(
  "noCre_rep1",
  "Control_Control_rep1", "Control_Control_rep2", "Control_Control_rep3", "Control_Control_rep4",
...
ggsave(file.path(Outdirectory, "Supplementary_Figure_4b.pdf"),
...
plot_groups   <- c("Control_Control", "ChR2_5min", "ChR2_30min")
...
PCA_batch_corrected <- ggplot(pca_df_plot, aes(PC1, PC2, color = Group, shape = Fraction)) +
  ...
  scale_color_manual(
    values = setNames(
      ...
      c("Control_Control", "ChR2_5min", "ChR2_30min")
    )
  )
PCA_batch_corrected
...
source(file.path("~/ruthasinger_github/OptoCLIP/R_functions/run_deseq_contrasts.R"))
...
if (length(stacked_tables_rt) > 0) {
  ...
  ggsave(
    filename = file.path(Outdirectory, "Figure_4g.pdf"),
...
for (comp in names(RT_out_list)) {
  ...
  RT_labeled_volcano_plots[[comp]] <- vol_labeled
...
ggsave(
  file.path(Outdirectory, "Figure_4e.pdf"),
  RT_labeled_volcano_plots[["5v0"]],
  width = 4.5,
  height = 4,
  units = "in"
)
...
ggsave(
  file.path(Outdirectory, "Figure_4f.pdf"),
  RT_labeled_volcano_plots[["30v5"]],
  width = 4.5,
  height = 4,
  units = "in"
)
...
if (length(RT_violin_tables_CH_NPvsCB) > 0) {
  ...
  ggplot2::ggsave(file.path(Outdirectory, "Figure_4i.pdf"),CH_all_violin_plot,width = 7, height = 4, units = "in"  )
}
...
pdf(file.path(Outdirectory, "Supplementary_Figure_4e.pdf"), width = 10, height = 6)
...
################################################################################################
# Test whether pathway-specific mRNAs regulated at 5 min reverse direction from 5 to 30 min

synaptic_5min_up_genes <- RT_out_list[["5v0"]]$nucsyn_up$gene_classification %>%
  dplyr::filter(category == "synaptic_terms") %>%
  dplyr::pull(gene) %>%
  unique()

nuclear_5min_down_genes <- RT_out_list[["5v0"]]$nucsyn_down$gene_classification %>%
  dplyr::filter(category == "nuclear_terms") %>%
  dplyr::pull(gene) %>%
  unique()

RT_5v0_results <- RT_out_list[["5v0"]]$res %>%
  dplyr::select(
    gene_name,
    RiboTag_log2FC,
    RiboTag_pvalue,
  ...
  dplyr::rename(
    log2FC_5v0 = RiboTag_log2FC,
    pvalue_5v0 = RiboTag_pvalue,
    padj_5v0   = RiboTag_padj
  )

RT_30v5_results <- RT_out_list[["30v5"]]$res %>%
  dplyr::select(
    gene_name,
    RiboTag_log2FC,
    RiboTag_pvalue,
  ...
  dplyr::rename(
    log2FC_30v5 = RiboTag_log2FC,
    pvalue_30v5 = RiboTag_pvalue,
    padj_30v5   = RiboTag_padj
  )

RT_temporal_results <- RT_5v0_results %>%
  dplyr::inner_join(
    RT_30v5_results,
    by = "gene_name"
  )

synaptic_temporal_df <- RT_temporal_results %>%
  dplyr::filter(gene_name %in% synaptic_5min_up_genes) %>%
  dplyr::mutate(
    pathway = "Synaptic: increased at 5 min",
...
nuclear_temporal_df <- RT_temporal_results %>%
  dplyr::filter(gene_name %in% nuclear_5min_down_genes) %>%
  dplyr::mutate(
    pathway = "Nuclear: decreased at 5 min",
...
RT_pathway_temporal_df <- dplyr::bind_rows(
  synaptic_temporal_df,
  nuclear_temporal_df
) %>%
  dplyr::mutate(
    pathway = factor(
      pathway,
      levels = c(
        "Synaptic: increased at 5 min",
        "Nuclear: decreased at 5 min"
      )
    ),
...
RT_pathway_temporal_df <- RT_pathway_temporal_df %>%
  dplyr::mutate(
    fill_group = dplyr::case_when(
      pathway == "Synaptic: increased at 5 min" &
        temporal_category == "Significant reversal" ~ "Synaptic reversal",
      
      pathway == "Synaptic: increased at 5 min" &
        temporal_category == "No significant reversal" ~ "Synaptic no reversal",
      
      pathway == "Nuclear: decreased at 5 min" &
        temporal_category == "Significant reversal" ~ "Nuclear reversal",
      
      pathway == "Nuclear: decreased at 5 min" &
        temporal_category == "No significant reversal" ~ "Nuclear no reversal"
    )
  )

RT_temporal_category_summary <- RT_pathway_temporal_df %>%
  dplyr::count(pathway, fill_group, name = "n_genes") %>%
  tidyr::complete(pathway, fill_group, fill = list(n_genes = 0)) %>%
  dplyr::group_by(pathway) %>%
  dplyr::mutate(
    total_genes = sum(n_genes),
    pct_genes = 100 * n_genes / total_genes
  ) %>%
  dplyr::ungroup()

temporal_fill_colors <- c(
  "Synaptic reversal"    = "#2E86DE",  # blue
  "Synaptic no reversal" = "#A9CCE3",  # light blue
  "Nuclear reversal"     = "#E74C3C",  # red
  "Nuclear no reversal"  = "#F5B7B1"   # light red
)

RT_temporal_summary <- RT_pathway_temporal_df %>%
  dplyr::group_by(pathway) %>%
  dplyr::summarise(
    n_5min_pathway_genes = dplyr::n(),
    
    n_significantly_reversed =
      sum(significant_reversal, na.rm = TRUE),
    
    pct_significantly_reversed =
      100 * n_significantly_reversed / n_5min_pathway_genes,
    
    n_no_significant_reversal =
      n_5min_pathway_genes - n_significantly_reversed,
    
    pct_no_significant_reversal =
      100 * n_no_significant_reversal / n_5min_pathway_genes,
...
RT_temporal_category_summary <- RT_pathway_temporal_df %>%
  ...
  dplyr::mutate(
    total_genes = sum(n_genes),
    pct_genes = dplyr::if_else(
      total_genes > 0,
      100 * n_genes / total_genes,
      0
    )
  ) %>%
  dplyr::ungroup()

RT_temporal_barplot <- ggplot(
  RT_temporal_category_summary,
  aes(
    x = pathway,
    y = pct_genes,
    fill = fill_group
...
print(RT_temporal_barplot)

ggsave(
  filename = file.path(
    Outdirectory,
    "Supplementary_Figure_4f.pdf"
  ),
  plot = RT_temporal_barplot,
  width = 7,
  height = 4.5,
  units = "in"
)
...
# This filtering step keep genes with >=2 tags in ALL replicates (4/4) within at least one condition
...
subset_vs_rest_p <- function(
  ...
  a <- length(intersect(subset_genes, target_genes))
  b <- length(setdiff(subset_genes, target_genes))
  c <- length(setdiff(target_genes, subset_genes))
  d <- length(
    setdiff(
      universe_genes,
      union(subset_genes, target_genes)
    )
  )
  ...
  exp_min <- suppressWarnings(
    min(chisq.test(mat)$expected)
  )
...
# Global FMRP target list
FMRP_targets_all <- unique(
  FMRP_STAR_TPM_wider$gene_name
)

RT_FMRP_enrichment_plots <- list()
enrich_summary <- list()
...
for (comp in names(RT_out_list)) {
  ...
  if (
    is.null(res) ||
    !("gene_name" %in% colnames(res))
  ) {
    
    message(
      "  - Missing res/gene_name for ",
      comp,
      "; skipping."
    )
  ...
  # Universe: all genes present in this RiboTag results table
  U <- unique(
    as.character(res$gene_name)
  )
  ...
  # Significant RiboTag subsets
  RT_up <- if (
    !is.null(up_df) &&
    "gene_name" %in% colnames(up_df)
  ) {
    
    unique(
      as.character(up_df$gene_name)
    )
  ...
  RT_down <- if (
    !is.null(down_df) &&
    "gene_name" %in% colnames(down_df)
  ) {
    
    unique(
      as.character(down_df$gene_name)
    )
  ...
  # Restrict all gene sets to the RiboTag universe
  RT_up <- intersect(
    RT_up,
    U
  )
  ...
  FMRP_targets <- intersect(
    FMRP_targets_all,
    U
  )
  
  
  # Raw enrichment p-values
  p_up_raw <- subset_vs_rest_p(
    RT_up,
    U,
    FMRP_targets
  )
  
  p_down_raw <- subset_vs_rest_p(
    RT_down,
    U,
    FMRP_targets
  )
  ...
  # BH correction across the two tests within this timepoint
  p_adj <- p.adjust(
    c(
      p_up_raw,
      p_down_raw
  ...
  # Build stacked percentage dataframe
  bar_df <- dplyr::bind_rows(
    
    tibble::tibble(
      group = "All RT expressed",
      gene = U
    ),
  ...
  # Plot labels use BH-adjusted p-values
  ...
  p <- ggplot2::ggplot() +
    ...
    ggplot2::labs(
      ...
      y = "Percent",
      title = paste0(
        "FMRP target enrichment: RiboTag ",
        comp
      )
    ) +
  ...
  RT_FMRP_enrichment_plots[[comp]] <- p
  
  print(p)

  enrich_summary[[comp]] <- tibble::tibble(
    
    comp = comp,
    
    universe_n = length(U),
    
    targets_in_universe_n = length(
      FMRP_targets
    ),
...
ggplot2::ggsave(
  filename = file.path(
    Outdirectory,
    "Figure_4j_5minvs0min.pdf"
  ),
  plot = RT_FMRP_enrichment_plots[["5v0"]],
  width = 4,
  height = 6,
  units = "in"
)
...
ggplot2::ggsave(
  filename = file.path(
    Outdirectory,
    "Figure_4j_30minvs5min.pdf"
  ),
  plot = RT_FMRP_enrichment_plots[["30v5"]],
  width = 4,
  height = 6,
  units = "in"
)
...
saveRDS(FMRP_RiboTag_merged, file=file.path("~/ruthasinger_github/OptoCLIP/RDS_files/FMRP_RiboTag_merged.rds"))

################################################################################################################################
#Source data

#Source data for Supplementary Figure 4b
SourceData_RiboTag_IPvsInput <- RiboTag_TPM_GOI_log2FC %>%
  dplyr::arrange(factor(gene_name, levels = GoI)) %>%
  dplyr::select(
    gene_name,
    noCre_rep1_log2FC,
    Control_Control_rep1_log2FC,
...
write.csv(
  SourceData_RiboTag_IPvsInput,
  file.path(Outdirectory, "Source_Data_Supplementary_Figure_4b.csv"),
  row.names = FALSE
)

#Source data for Fig. 4e,f
RT_volcano_source_data <- purrr::map_dfr(
  names(RT_out_list),
  function(comp) {
    ...
    up_label_genes <- get_genes_from_go_terms(
      ...
      res_df = res,
      lfc_col = "RiboTag_log2FC",
      pvalue_col = "RiboTag_padj",
      geneid_col = geneid_col,
      gene_sep = gene_sep,
      top_n = spec$volcano_label_top_n,
      rank_by = "pvalue"
    ...
    res %>%
      dplyr::transmute(
        ...
        Volcano_label = dplyr::case_when(
          gene_name %in% up_label_genes   ~ "Labeled increased gene",
          gene_name %in% down_label_genes ~ "Labeled decreased gene",
          TRUE                            ~ "Not labeled"
        )
      )
  }
...
readr::write_excel_csv(
  RT_volcano_source_data,
  file.path(
    Outdirectory,
    "Source_Data_Figure_4e_4f.csv"
  )
)

#Source data for Figure 4d
RT_stackedbar_gene_data <- purrr::map_dfr(
  names(RT_out_list),
  function(comp) {
    
    up_genes <- RT_out_list[[comp]]$nucsyn_up$gene_classification %>%
      dplyr::mutate(
        Comparison = comp,
        Direction = "Up"
      )
    
    down_genes <- RT_out_list[[comp]]$nucsyn_down$gene_classification %>%
      dplyr::mutate(
        Comparison = comp,
        Direction = "Down"
      )
...
readr::write_excel_csv(
  RT_stackedbar_gene_data,
  file.path(
    Outdirectory,
    "Source_Data_Figure_4g.csv"
  )
)

#Source data for Figure 4f
RT_temporal_reversal_gene_data <- RT_pathway_temporal_df %>%
  dplyr::transmute(
    gene_name,
    Pathway_group = as.character(pathway),
    log2FC_5v0,
    pvalue_5v0,
    padj_5v0,
    log2FC_30v5,
    pvalue_30v5,
    padj_30v5,
    Significant_reversal = significant_reversal,
    Temporal_category = as.character(temporal_category),
    Plot_category = fill_group
  ) %>%
  dplyr::arrange(
    Pathway_group,
    Temporal_category,
...
readr::write_excel_csv(
  RT_temporal_reversal_gene_data,
  file.path(
    Outdirectory,
    "Source_Data_Supplementary_Fig_4f.csv"
  )
)

#Source Data for Figure 4h
RT_go_bubble_source_data <- RT_go_bubble_plot_df %>%
  dplyr::mutate(
  ...
  dplyr::rename(
    ...
    Negative_log10_adjusted_p_value = neglog10_padj_raw,
    Plotted_negative_log10_adjusted_p_value = neglog10_padj
  )

readr::write_excel_csv(
  RT_go_bubble_source_data,
  file.path(
    Outdirectory,
    "Source_Data_Figure_4h.csv"
  )
)

#Source Data for Figure 4i
CH_violin_source_data <- CH_all_violin_df %>%
  dplyr::mutate(
    comp = as.character(comp),
    Group = as.character(Group),
    Group_simple = as.character(Group_simple)
  ) %>%
  dplyr::select(
    dplyr::any_of(c(
      "gene_name",
  ...
  dplyr::rename(
    Comparison = comp,
    RiboTag_group = Group,
    Regulation_direction = Group_simple,
    Hale_CA1_NP_vs_CB_log2FC = CH_NPvsCB_log2FC
  )

readr::write_excel_csv(
  CH_violin_source_data,
  file.path(
    Outdirectory,
    "Source_Data_Figure_4i.csv"
  )
)

CH_violin_stats <- pw_facet %>%
  dplyr::mutate(
    comp = as.character(comp)
  ) %>%
  dplyr::select(
...
readr::write_excel_csv(
  ...
  file.path(
    Outdirectory,
    "Source_Data_Figure_4i_stats.csv"
  )
)

#Source data for Figure 4j 
RiboTag_CLIP_enrich_summary <- dplyr::bind_rows(
  enrich_summary
)

readr::write_excel_csv(
  RiboTag_CLIP_enrich_summary,
  file.path(
    Outdirectory,
...
readr::write_excel_csv(
  ...
  file.path(
    Outdirectory,
    "Source_Data_Supplementary_Figure_4e.csv"
  )
)

#Source data for Supplementary_Fig_4f
RT_temporal_reversal_gene_data <- RT_pathway_temporal_df %>%
  dplyr::transmute(
    gene_name,
    Pathway_group = as.character(pathway),
...
readr::write_excel_csv(
  ...
  file.path(
    Outdirectory,
    "Source_Data_Supplementary_Figure_4f.csv"
  )
)
