...
Base_directory=file.path("~/ruthasinger_github/OptoCLIP/Figure3_FigureS3_FMRP_CLIP")
...
ggsave(file.path(Outdirectory, "Figure_3d.pdf"), plot = FMRP_CLIP_STAR_tx_per_condition_bar_plot_stats,width = 6, height = 6)
...
pdf(file = file.path(Outdirectory,"Supplementary_Figure_3a.pdf"))
...
meta_plot
ggsave(file.path(Outdirectory, "Figure_3f.pdf"), meta_plot, width = 5, height = 5)
...
pairwise_stats_sig <- region_ratios_all %>%
...
print(pairwise_stats_sig[,c(1,3,4,5,6,7,8,9,10)])
...
ggsave(file.path(Outdirectory, "Figure_3g.pdf"),plot_regions_box_all, width = 5, height = 5)
...
ggsave(file.path(Outdirectory, "Figure_3e.pdf"),plot = FMRP_CLIP_STAR_byregion_BC_plot, width = 6, height = 5)

################################################################################################################################################################
#Source data- these will write csv files that are compiled in one Source Data file included in the paper

#Source Data for Figure 3d
Source_Data_Figure_3d <- df_counts_tx_sample %>%
  mutate(Condition = as.character(Condition_plot)) %>%
  dplyr::select(
    Sample,
    Condition,
    n_tags
  ...
  rename(
    `FMRP CLIP transcriptome tags` = n_tags
  ) %>%
  arrange(Condition, Sample)

# Statistics for Fig. 3d
Source_Data_Figure_3d_Stats <- all_pairwise_tbl %>%
  dplyr::select(
    group1,
    group2,
    n1,
...
outfile <- file.path(Outdirectory, "Source_Data_Figure_3d.csv")

write.csv(
  Source_Data_Figure_3d,
  outfile,
  row.names = FALSE
)
...
write(
  "Pairwise t-test statistics",
  file = outfile,
  append = TRUE
)

write.table(
  Source_Data_Figure_3d_Stats,
  outfile,
  sep = ",",
  row.names = FALSE,
  col.names = TRUE,
...
Source_Data_Figure_3g <- region_ratios_all %>%
  ...
  dplyr::rename(
    ...
    `Condition 1` = group1,
    `Condition 2` = group2,
    `Number of transcripts (Condition 1)` = n1,
    `Number of transcripts (Condition 2)` = n2,
